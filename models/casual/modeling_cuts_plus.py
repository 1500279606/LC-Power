# adapted from: https://github.com/jarrycyx/UNN
import torch
...
class Causal_Net(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        n_nodes = cfg.CUTS_PLUS.N_NODES
        in_ch = cfg.CUTS_PLUS.DATA_DIM
        hidden_ch = cfg.CUTS_PLUS.DATA_PRED.MLP_HID
        shared_weights_decoder = cfg.CUTS_PLUS.DATA_PRED.SHARED_WEIGHTS_DECODER
        concat_h = cfg.CUTS_PLUS.DATA_PRED.CONCAT_H

        self.in_ch = cfg.CUTS_PLUS.DATA_DIM
        self.hidden_ch = cfg.CUTS_PLUS.DATA_PRED.MLP_HID
        self.n_layers = cfg.CUTS_PLUS.DATA_PRED.GRU_LAYERS
    ...
    def forward(self, x, fwd_graph):
        ...
        bs, n_nodes, steps = x.shape
        
        h = [h_.expand(bs, -1, -1) for h_ in self.h0.to(DEVICE)]
        
        pred = []
        for step in range(steps):
            x_now = x[..., step].unsqueeze(1) # [batches, 1, nodes]
        ...
        # 只返回最后 PRED_STEP 步的预测
        # 用于匹配训练数据中的 y 维度
        return pred[:, :, -self.cfg.CUTS_PLUS.DATA_PRED.PRED_STEP:]
