...
class PositiveAugmentor(nn.Module):
	def __init__(self, cfg):
		...
		self.cfg = cfg
		self.cfg_positive_augmentor = cfg.Causal.POSITIVE_AUGMENTOR
	...
	def forward(self, x, causality_mtx):
		...
		x_pos[:, self.cfg_causal_discoverer.INPUT_STEP, start_node] += torch.randn(x.size(0)).to(DEVICE) * self.cfg_positive_augmentor.NOISE_LEVEL
		with torch.no_grad():
		...
		# x_out: (B, PRED_STEP, N) - 复制到最后 PRED_STEP 步
		x_pos[:, -self.cfg_causal_discoverer.DATA_PRED.PRED_STEP:, effects] = x_out[:, :, effects]
