...
def loss_fn(x, cfg):
    ...
    indices = torch.where(sim_mtx_pos < cfg.Causal.SIM_THRESHOLD)
    ...
    sim_mtx = torch.exp(sim_mtx / cfg.Causal.TEMPERATURE)
    ...
    # Compute the loss
    loss = torch.sum(sim_mtx[torch.where(sim_mtx_pos >= cfg.Causal.SIM_THRESHOLD)]) / (len(torch.where(sim_mtx_pos >= cfg.Causal.SIM_THRESHOLD)[0]) - n)
