...
def batch_generater(data, observ_mask, bs, n_nodes, input_step, pred_step, block_size=None):
    ...
    for batch_i in range(len(random_t_list) // bs):
        x = torch.zeros([bs, n_nodes, input_step]).to(DEVICE)
        y = torch.zeros([bs, n_nodes, pred_step]).to(DEVICE)
        t = torch.zeros([bs]).to(DEVICE).long()
        mask_x = torch.zeros([bs, n_nodes, input_step]).to(DEVICE)
        mask_y = torch.zeros([bs, n_nodes, pred_step]).to(DEVICE)
...
class CUTS_PLUS_Trainer(Trainer):
    ...
    def latent_data_pred(self, x, y):
        ...
        graph_sampled = sample_bernoulli(Graph, bs)  # 使用实际 batch size 而不是固定的 TRAIN.BATCH_SIZE
    ...
    def graph_discov(self, x, y):
        ...
        Graph = torch.einsum("nm,ml->nl", G_prob, torch.sigmoid(GT_prob))
        graph_sampled = gumbel_sigmoid_sample(Graph, bs)  # 使用实际 batch size 
    ...
    def train(self):
        ...
        for epoch_i in range(self.cfg_model.SOLVER.MAX_EPOCH):
            self.G = torch.eye(self.cfg_model.N_NODES).to(DEVICE)
