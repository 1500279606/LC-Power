...
_C.SPARSETSFE = CN()
# Sparse Encoder 配置
_C.SPARSETSFE.D_MODEL = 512        # 输出特征维度
_C.SPARSETSFE.DROPOUT = 0.1        # dropout 比例
_C.SPARSETSFE.MODEL_TYPE = 'mlp'   # 'linear' 或 'mlp'
# 周期长度，用于下采样和上采样
_C.SPARSETSFE.PERIOD_LEN = 2       # seq_len // period_len = seg_num_x


# LC-Power 模型配置（与 Causal 共享）
_C.LC-Power = CN()
_C.LC-Power.METRIC_NAMES = ("Loss",)
_C.LC-Power.LOSS_NAMES = ("Loss",)


_C.Causal = CN()
_C.Causal.CAUSALITY_THRESHOLD = 0.5
# lstm, iTransformer, GATv2, TimesNet, gru, sparse
_C.Causal.ENCODER_ARCH = 'sparse'
_C.Causal.METRIC_NAMES = ("Loss",)
_C.Causal.LOSS_NAMES = ("Loss",)
_C.Causal.TEMPERATURE = 0.1
_C.Causal.SIM_THRESHOLD = 0.5
_C.Causal.SIM_THRESHOLD_SCHEDULE = True
_C.Causal.SIM_THRESHOLD_START = 0.5
_C.Causal.SIM_THRESHOLD_END = 0.9

_C.Causal.PROJECTOR = CN()
_C.Causal.PROJECTOR.INPUT_DIM = 512
_C.Causal.PROJECTOR.HIDDEN_DIM = 1024
_C.Causal.PROJECTOR.OUTPUT_DIM = 512

_C.Causal.POSITIVE_AUGMENTOR = CN()
_C.Causal.POSITIVE_AUGMENTOR.ENABLE = True
_C.Causal.POSITIVE_AUGMENTOR.NOISE_LEVEL = 0.1

_C.Causal.NEGATIVE_AUGMENTOR = CN()
_C.Causal.NEGATIVE_AUGMENTOR.TRANSFORM_NAME = "AddBias"
_C.Causal.NEGATIVE_AUGMENTOR.CUTOFF_PROBABILITY = 0.1
_C.Causal.NEGATIVE_AUGMENTOR.DISTURB_ALL = False
